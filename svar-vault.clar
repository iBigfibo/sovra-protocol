;; svar-vault.clar - Argentina Sovereign Bond Vault (40% APR)
...
(define-constant BASE_APR u4000)  ;; 40%
...
(define-public (deposit (usdc-amount uint) (lock-days uint))
  (let
    ...
    (try! (ft-mint? svAR usdc-amount caller))
...
(define-public (redeem)
  (let
    (
      ...
      (shares (ft-get-balance svAR caller))
    )
    (asserts! (> shares u0) ERR-INSUFFICIENT-BALANCE)
    (try! (ft-burn? svAR shares caller))
