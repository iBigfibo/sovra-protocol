;; svng-vault.clar - Nigeria Sovereign Bond Vault (20% APR)
...
(define-constant BASE_APR u2000)  ;; 20%
...
(define-public (deposit (usdc-amount uint) (lock-days uint))
  (let
    ...
    (try! (ft-mint? svNG usdc-amount caller))
...
(define-public (redeem)
  (let
    (
      ...
      (shares (ft-get-balance svNG caller))
    )
    (asserts! (> shares u0) ERR-INSUFFICIENT-BALANCE)
    (try! (ft-burn? svNG shares caller))
